...
public class ModifyUserChromeDemo {
	...
	private static void ModifyUserName()
	{
		try 
		{
			oPage.getclickonModifiedUsername().click();
		Thread.sleep(3000);
		oPage.getfirstNameInUser().clear();
		oPage.getfirstNameInUser().sendKeys("RemodifiedDemo");
		oPage.getlastNameinUser().clear();
		oPage.getlastNameinUser().sendKeys("d2");
		oPage.getusernameFieldinUsername().clear();
		oPage.getusernameFieldinUsername().sendKeys("Demod2");
		oPage.getclickmodifiedSaveChanges().click();
		Thread.sleep(3000);
